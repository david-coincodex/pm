 ...
 *
 * Never cam data either: cam-models and cam-favorites are ENVIRONMENT-LOCAL (the registry is
 * regenerated per host by the roster sync + crons; favorites belong to that host's users).
 * Both transfer legs run through scripts/strapi-transfer-nocam.cjs, which filters them out of
 * the stream AND spares them from the destination's pre-restore deletion. See that file and
 * the "Cam data" section of the .md before touching either.
 *
...
async function runApply(proxyPort, snapshotTs) {
  ...
  // Both legs run through the nocam wrapper: cam-models/cam-favorites are environment-local
  // (regenerated by the roster sync + crons) and must neither ship with a transfer nor be
  // DELETED on the destination by the transfer's restore step. The wrapper self-checks first
  // and refuses to run an unfiltered transfer if the Strapi internals it patches have moved.
  const WRAPPER = join(__dirname, 'strapi-transfer-nocam.cjs');
  const WRAPPER_IN = '/tmp/pm-transfer-nocam.cjs';
  dc(['cp', WRAPPER, `backend:${WRAPPER_IN}`]);
  inBackend(`node ${WRAPPER_IN} --selfcheck`);

  if (!NO_SNAPSHOT) {
    console.log(`\n── Snapshot: pulling staging into pmsnap-${snapshotTs} ──────────`);
    await sh('docker', [
      'compose', '-f', 'docker-compose.backup.yml', '-p', `pmsnap-${snapshotTs}`, 'run', '--rm',
      '-v', `${WRAPPER}:${WRAPPER_IN}:ro`, 'scratch',
      'node', WRAPPER_IN, 'transfer',
  ...
  await sh('docker', [
    ...
    'node', WRAPPER_IN, 'transfer', ...to, '--only', ONLY, '--force',
  ]);
}
