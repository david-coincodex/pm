...
export function syncModels(snapshot: OnlineSnapshot): void {
  ...
  void fetch(`${STRAPI_FETCH_URL}/api/cam-models/sync`, {
    ...
    // failedProviders carries the SUSTAINED failures — feeds that have failed every refresh
    // for 10 minutes — not this cycle's misses. The backend's roster-sync heartbeat pings
    // /fail on it, so a single blip must not appear here: retention serves last-known models
    // between polls, so one miss is invisible to visitors and not actionable, and paging on it
    // produced alerts that were already resolved by the time anyone read them. A feed still
    // down after 10 minutes is a rotated key or a changed endpoint, and that still pages.
    // (Field name kept: it is the backend's request contract. NOT degradedProviders, which is
    // the UI-banner field and stays empty while retention hides a warm outage.) The check's
    // period lives in backend/src/cron/checks.json and assumes SYNC_INTERVAL_MS ≈ 5 min —
    // retune both together.
    body: JSON.stringify({ models, failedProviders: snapshot.sustainedFailures }),
    cache: 'no-store',
