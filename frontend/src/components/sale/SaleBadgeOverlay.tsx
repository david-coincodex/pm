...
import type { StrapiMedia, SaleBadgeIcon } from '@/lib/strapi';
...
export default function SaleBadgeOverlay({
  badgeImage,
  badgeLabel,
  badgeIcon,
  themeColor,
  ...
  if (badgeImage) {
    return (
      <div className="absolute left-2 top-2 h-10 w-auto">
        <Image
          src={strapiMediaUrl(badgeImage)}
          alt={badgeImage.alternativeText ?? 'Sale badge'}
  ...
  return (
    <span
      className="absolute left-2 top-2 inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-bold text-white shadow"
